...
df2 = []
losses2 = []
for run in api.runs("linx/markov-LLM-order-depth-final-l2", {"config.order": 2}):
    try:
...
df4 = []
losses4 = []
for run in api.runs("linx/markov-LLM-order-depth-final-l2", {"config.order": 4}):
    try:
...
fig.savefig("loss2.pdf", bbox_inches='tight')