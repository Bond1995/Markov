...
res = []
for run in api.runs("markov-PQ-steady"):
    try:
        data = run.history()
        c = data["batch10"].values[:]
        res.append((run.config.get("p", None), run.config.get("q", None), np.std(c[~(np.isnan(c))])))
    except:
        pass
...
v = ax.matshow(piv, vmin=0, vmax=0.1)
ax.set_xticks(range(len(piv.columns)), piv.columns)
ax.set_yticks(range(len(piv.index)), piv.index)
ax.xaxis.set_ticks_position("bottom")
ax.set(xlabel="q", ylabel="p")
ax.set_title("Markov LLM estimated q standard deviation (6k iterations, steady state)")
fig.colorbar(v, ax=ax, label="Estimated q standard deviation")
for i in range(len(piv.index)):
    for j in range(len(piv.columns)):
        ax.text(j, i, f"{piv.iloc[i, j]:.3f}", ha="center", va="center", color="w")
fig.savefig("markov-std-q-steady.png", dpi=300)
