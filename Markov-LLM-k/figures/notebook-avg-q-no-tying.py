...
res = []
for run in api.runs("markov-no-tying"):
    try:
        data = run.history()
...
ax.set_title("Markov LLM average estimated q (400 iterations, no weight tying)")
fig.colorbar(v, ax=ax, label="Average estimated q")
for i in range(len(piv.index)):
    for j in range(len(piv.columns)):
        ax.text(j, i, f"{piv.iloc[i, j]:.3f}", ha="center", va="center", color="w")
fig.savefig("markov-avg-q-no-tying.png", dpi=300)
