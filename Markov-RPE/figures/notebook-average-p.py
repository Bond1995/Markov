...
res = []
for run in api.runs("markov-PQ-p"):
    try:
        data = run.history()
        c = data["batch1-p"].values[:]
...
ax.set_title("Markov LLM average estimated p (6k iterations, steady state)")
fig.colorbar(v, ax=ax, label="Average estimated p")
for i in range(len(piv.index)):
    for j in range(len(piv.columns)):
        ax.text(j, i, f"{piv.iloc[i, j]:.3f}", ha="center", va="center", color="w")
fig.savefig("markov-average-p-steady.png", dpi=300)
