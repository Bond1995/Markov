...
res = []
for run in api.runs("markov-simple-PQ-tying-unif"):
    try:
        data = run.history()
...
ax.set_title("Simple LLM average estimated q (400 iterations, with weight tying, uniform dist.)")
fig.colorbar(v, ax=ax, label="Average estimated q")
for i in range(len(piv.index)):
    for j in range(len(piv.columns)):
        ax.text(j, i, f"{piv.iloc[i, j]:.3f}", ha="center", va="center", color="w")
fig.savefig("simple-avg-q-tying-unif.png", dpi=300)
