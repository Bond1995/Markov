...
def train_base(model, opt, p, order, vocab_size, scheduler, iterations, acc_steps, batch_size, sequence_length, generator, eval_freq, ckpt_path, distributed_backend, extra_args):
    ...
    while itr < iterations:
        for microstep_idx in range(acc_steps):  # gradient accumulation
            x, y = get_batch(p, order, vocab_size, sequence_length, batch_size, generator, extra_args, device=extra_args.device)
            with type_ctx:
        ...
        if itr % eval_freq == 0 or itr == iterations: # from here it's only evaluation code, all the training is above
            if distributed_backend.is_master_process():
                ...
                val_acc, val_loss, val_perplexity = eval(model, p, order, vocab_size, sequence_length, batch_size,
                ...
                if itr == iterations:
                    _, _, _, prob_vec = eval_probs(model, p, order, vocab_size, sequence_length, generator, extra_args,
                                                         extra_args.device, ctx=type_ctx)
                    if extra_args.wandb:
                        for m in range(vocab_size):
                            for i in range(len(prob_vec[m])):
                                wandb.log({
                                    "est/est_" + str(m): prob_vec[m][i].detach().cpu().item(),
                                })
