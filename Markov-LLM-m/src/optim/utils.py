...
def get_batch(p, order, vocab_size, seq_length, batch_size, generator, extra_args, device='cpu'):
    ...
    data[:,:order] = torch.multinomial(alpha*torch.ones((batch_size,vocab_size), device=device), order, replacement=True, generator=generator)
    for i in range(order, seq_length):
        data[:,i] = get_next_symbols(p, vocab_size, data[:,i-order])
...
def get_next_symbols(p, vocab_size, data):
    P = (p / vocab_size) * torch.ones((vocab_size, vocab_size)) + (1 - 1.5*p) * torch.eye(vocab_size)
    P = P.to(data.device)
    M = P[data.to(int)]
...
@torch.no_grad()
def eval(model, p, order, vocab_size, sequence_length, batch_size, generator, extra_args, device='cpu', max_num_batches=24, ctx=nullcontext()):
    assert model.training == False

    loss_list_val, acc_list = [], []

    for _ in range(max_num_batches): 
        x, y = get_batch(p, order, vocab_size, sequence_length, batch_size, generator, extra_args, device=device)
        with ctx:
...
@torch.no_grad()
def eval_probs(model, p, order, vocab_size, sequence_length, generator, extra_args, device='cpu', ctx=nullcontext()):
    assert model.training == False

    loss_list_val, acc_list = [], []

    x, y = get_batch(p, order, vocab_size, sequence_length, 1, generator, extra_args, device=device)
    with ctx:
        outputs = model (x, targets=y, get_logits=True)
    val_loss = outputs['loss']
    loss_list_val.append(val_loss)
    ...
    for m in range(vocab_size):
        vec = probsb[idx == m][:, (m+1) % vocab_size] # estimated transition probability [m] -> [m+1]
        prob_vec.append(vec)
